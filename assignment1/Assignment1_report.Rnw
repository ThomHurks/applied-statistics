...
Even if we cannot assume normality of the clean batch, there are still ways to determine wether two populations differ. One such methods is the Wilcoxon Signed-Rank Test. To apply this test, the following assumptions are made:
...
<<echo=FALSE, warning=FALSE, message=FALSE, results="asis">>=
wilcoxB1 = unlist(wilcox.test(batchB0$OUTCOME, batchB1$OUTCOME, paired=TRUE, correct=FALSE, alternative="less")["p.value"])
wilcoxB2 = unlist(wilcox.test(batchB0$OUTCOME, batchB2$OUTCOME, paired=TRUE, correct=FALSE, alternative="less")["p.value"])
wilcoxB3 = unlist(wilcox.test(batchB0$OUTCOME, batchB3$OUTCOME, paired=TRUE, correct=FALSE, alternative="less")["p.value"])
wilcoxB4 = unlist(wilcox.test(batchB0$OUTCOME, batchB4$OUTCOME, paired=TRUE, correct=FALSE, alternative="less")["p.value"])
wilcoxB5 = unlist(wilcox.test(batchB0$OUTCOME, batchB5$OUTCOME, paired=TRUE, correct=FALSE, alternative="less")["p.value"])
wilcoxB6 = unlist(wilcox.test(batchB0$OUTCOME, batchB6$OUTCOME, paired=TRUE, correct=FALSE, alternative="less")["p.value"])
wilcoxB7 = unlist(wilcox.test(batchB0$OUTCOME, batchB7full$OUTCOME, paired=TRUE, correct=FALSE, alternative="less")["p.value"])
...
B2, B3 and B7, test as potentially tainted if we take $\alpha =0.05$. That is, it is unlikely that the higher sample values from these batches are due to sample bias. Applying Bonferroni correction, however, results in the null hypthesis holding for all potentially tainted batches ($p<0.01$). Note that the missing value in B7 is omitted in the test. 

\subsubsection{Kolgomorov-Smirnov Test}

Another test to compare the clean batch with the other batches without any assumption of normality is the Kolgomorov-Smirnov Test. For the 2-sample version of this test, we need to make the following assumptions:
...
Note that these are much less stringent than the assumptions required for the Wilcoxon Signed-Rank Test.

Contamination is caused by a leakage, so we expect tainted batches to generally have higher sample values. We perform a one-sided Wilcoxon Signed-Rank Test with the clean batch and each of the potentially tainted batches:

<<echo=FALSE, warning=FALSE, message=FALSE, results="asis">>=
ksB1 = unlist(ks.test(batchB0$OUTCOME, batchB1$OUTCOME, alternative="greater")["p.value"])
ksB2 = unlist(ks.test(batchB0$OUTCOME, batchB2$OUTCOME, alternative="greater")["p.value"])
ksB3 = unlist(ks.test(batchB0$OUTCOME, batchB3$OUTCOME, alternative="greater")["p.value"])
ksB4 = unlist(ks.test(batchB0$OUTCOME, batchB4$OUTCOME, alternative="greater")["p.value"])
ksB5 = unlist(ks.test(batchB0$OUTCOME, batchB5$OUTCOME, alternative="greater")["p.value"])
ksB6 = unlist(ks.test(batchB0$OUTCOME, batchB6$OUTCOME, alternative="greater")["p.value"])
ksB7 = unlist(ks.test(batchB0$OUTCOME, batchB7full$OUTCOME, alternative="greater")["p.value"])
