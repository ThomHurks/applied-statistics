...
\end{cases}
\end{align}
We will reject the null hypothesis if p-value $< 0.05$. Besides Barlett's test we will perform an F-test to see which batches have similar properties as batch 0. This can be formulated as can be found in equation \ref{ftest}:
\begin{align}
\label{ftest}
\begin{cases}
