...
\subsection{Exploritorary data analysis}
...
Besides performing a t-test, we will also perform some homogeneity tests to see whether the data also share the same characteristics. To check if this is the case, we will perform a Barlett's test, which checks whether all the variances of the batches are equal to each other. The null hypothesis of that test is that all variances are equal, and thus that the sets are homogeneous. Besides that we will perform an F-test to see which batches have similar properties as batch 0.
...
H_1: \text{Batch}_0 \text{ and batch}_i \text{ come from a different population} & \quad \text{for } 1 \leq i \leq 7 \\
\end{cases}
\end{align}
...
\subsubsection{T-Test}
As can be found in the table above containing the variance of the batches, we can see that the variances of the batches are not equal. We therefore have to perform a t-test that assumes unequal variances. Batch seven has one missing value, which should not be a problem for a t-test.\\
...
The other pitfall of our investigation is that we have only performed some conservative tests. Because we had to adjust the critical value, we increase the change that a type II error would occur. This could mean we accepted a batch as being clean, while in fact it is affected by silicones. To get more reliable statistical results, it could be wise to perform some ANOVA tests, to see whether those tests detect the same batch as affected.
\end{document}
