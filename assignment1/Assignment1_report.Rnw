...
Even if we cannot assume normality of the clean batch, there are still ways to determine wether two populations differ. One such methods is the Kolgomorov-Smirnov Test. For the 2-sample version of this test, we need to make the following assumptions:
...
We can make these assumptions based on the assignment description. We perform a 2-sided test to get a general view of wether non-parametric testing can pick up anything.
...
B3 and B5 test as potentially tainted if we take $\alpha =0.05$. Applying a somewhat relaxed Bonferroni correction ($p<0.01$), however, results in the null hypthesis holding for all potentially tainted batches. Note that the missing value in B7 is omitted in the test with B7 and replaced with the mean of all other values in B7 in the test with B7*. 
