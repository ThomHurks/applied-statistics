...
\subsection{Outliers}
We will perform a Grubbs outlier test to check whether there are any indications that one of the measurements has some incorrect value due to for example measurement errors.
\section{Results}
