...
Some statistical tests assume normality of the data, or require the data to at least approach normality. For that reason we will investige if the data is normally distributed by inspecting normal QQ-plots and by using the Shapiro-Wilk test. If necessary we will transform the data in such a way that the transformed data is normally distributed or nearly normal. For non-parametric tests that do not assume any specific distribution we will use the original non-transformed data. The Shapiro-Wilk hypothesis can be found in equation \ref{normal}. We will reject the null hypothesis if the p-value $<0.01$.
Besides a Shapiro-Wilk test, we will perform skwewness and kurtosis tests to determine where normality can be rejected. The critical value of a two-sided skewness test is 1.157, while the critical value of the kurtosis is 1.49. We will perform a Jarque-Bera test to see if it is appropriate to reject a normal distribution based on skewness and kurtosis. The null hypothesis is the same as with the Shapiro-Wilk test which can be found in equation \ref{normal}. We will reject normality if the p-value $< 0.01$.
