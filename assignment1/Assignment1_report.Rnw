...
library(mice)
imputed_dataset = complete(mice(dataset), 1)
dataset = dataset[!is.nan(dataset$OUTCOME),]
dataset$LOGOUTCOME = log(dataset$OUTCOME)
imputed_dataset$LOGOUTCOME = log(imputed_dataset$OUTCOME)
dataset = dataset[!is.nan(dataset$OUTCOME),]
...
batchB7_imputed = imputed_dataset[imputed_dataset$BATCH == "B7",]
...
<<echo=FALSE, warning=FALSE, message=FALSE, results="asis">>=
var_log_B7 = var(batchB7$LOGOUTCOME)
variance_log_frame = data.frame(batch=c("B0", "B1", "B2", "B3", "B4", "B5", "B6", "B7"), 
                                "variance"=c(var(batchB0$LOGOUTCOME), var(batchB1$LOGOUTCOME),
                                           var(batchB2$LOGOUTCOME), var(batchB3$LOGOUTCOME),
                                           var(batchB4$LOGOUTCOME), var(batchB5$LOGOUTCOME),
                                           var(batchB6$LOGOUTCOME), var_log_B7))
library(xtable)
...
for (batch in batches) {
  Bi = dataset[dataset$BATCH == batch,]$LOGOUTCOME
  vartestvalues[batch] <- var.test(batchB0$LOGOUTCOME, Bi)$p.value
}
...
ksB7m = unlist(ks.test(batchB0$OUTCOME, batchB7_imputed$OUTCOME, exact=TRUE)["p.value"])
...
wilcoxB7 = unlist(wilcox.test(batchB0$OUTCOME, batchB7$OUTCOME, paired=FALSE, correct=FALSE, exact=TRUE, alternative="less")["p.value"])
wilcoxB7m = unlist(wilcox.test(batchB0$OUTCOME, batchB7_imputed$OUTCOME, paired=FALSE, correct=FALSE, exact=TRUE, alternative="less")["p.value"])
...
@
Since we reject batches if the p-value $< 0.01$, only batch 3 fails to pass the null hypothesis. The other batches do not deviate much from batch zero. 
\subsection{Outliers}
<<echo=FALSE, warning=FALSE, message=FALSE, results="asis">>=
library(outliers)
