...
\subsubsection{Hampel's rule}
<<echo=FALSE>>=
for (i in 1:length(dataset_normality))
{
  anv = abs(dataset_normality[i] - median(dataset_normality))/mad(dataset_normality);
  if (anv > 3.5) { print(dataset_normality[i]); }
}
@
If we compute the absolute normalized value according to Hampel's rule for each observation, then we see that none of the absolute normalized values are larger than $3.5$ and as such none are considered outliers according to Hampel's rule. The largest computed absolute normalized value is $\Sexpr{abs(dataset_normality[2] - median(dataset_normality))/mad(dataset_normality)} < 3.5$ for observation $\Sexpr{dataset_normality[2]}$.
\subsubsection{Tukey's method}
<<echo=FALSE>>=
...
\subsubsection{Hampel's rule}
<<echo=FALSE>>=
for (i in 1:length(dataset_normality_log))
{
  anv = abs(dataset_normality_log[i] - median(dataset_normality_log))/mad(dataset_normality_log);
  if (anv > 3.5) { print(dataset_normality_log[i]); }
}
@
If we compute the absolute normalized value according to Hampel's rule for each observation, then we see that none of the absolute normalized values are larger than $3.5$ and as such none are considered outliers according to Hampel's rule. The largest computed absolute normalized value is $\Sexpr{abs(dataset_normality_log[2] - median(dataset_normality_log))/mad(dataset_normality_log)} < 3.5$ for observation $\Sexpr{dataset_normality_log[2]}$.
\subsubsection{Tukey's method}
